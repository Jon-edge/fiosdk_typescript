...
class FIOSDK {
            ...
            apply: function (target, scope, args) {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (FIOSDK.customRawAbiAccountName) {
                        rawAbiAccountNameList = [...constants_1.Constants.rawAbiAccountName, ...FIOSDK.customRawAbiAccountName];
                    }
                    else {
                        rawAbiAccountNameList = constants_1.Constants.rawAbiAccountName;
                    }
                    ...
                    yield Promise.allSettled(setAbiPromises).then(results => results.forEach(result => {
                        ...
                        if (result.status === 'rejected') {
                            ...
                            if (error.includes(constants_1.Constants.missingAbiError)) {
                                ...
                                if (!((_c = this.main.rawAbiMissingWarnings) === null || _c === void 0 ? void 0 : _c.includes(abiAccountName)) || (FIOSDK.customRawAbiAccountName && FIOSDK.customRawAbiAccountName.includes(abiAccountName))) {
    ...
    **/
    static setRawAbiMissingWarnings(rawAbiName, fioSdkInctance) {
    ...
    executePreparedTrx(endPoint, preparedTrx) {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield this.transactions.multicastServers({ endpoint: `chain/${endPoint}`, body: JSON.stringify(preparedTrx) });
    ...
    registerFioDomainAddress(options) {
        const registerFioDomainAddress = new SignedTransactions.RegisterFioDomainAddress(Object.assign(Object.assign({}, options), { technologyProviderId: this.getTechnologyProviderId(options.technologyProviderId) }));
    ...
    recordObtData(options) {
        return __awaiter(this, void 0, void 0, function* () {
            const { amount, chainCode, encryptPrivateKey = null, fioRequestId = null, hash = null, maxFee, memo = null, obtId, offLineUrl = null, payeeFioAddress, payeeFioPublicKey = '', payeeTokenPublicAddress, payerFioAddress, payerTokenPublicAddress, status, technologyProviderId = null, tokenCode, } = options;
            let payeeKey = '';
            const encryptKey = yield this.getEncryptKey(payeeFioAddress);
    ...
    requestFunds(options) {
        return __awaiter(this, void 0, void 0, function* () {
            const { amount, chainCode, encryptPrivateKey = null, hash = null, maxFee, memo, offlineUrl = null, payeeFioAddress, payeeTokenPublicAddress, payerFioAddress, payerFioPublicKey = null, technologyProviderId = null, tokenCode, } = options;
            let payerKey = '';
            const encryptKey = yield this.getEncryptKey(payerFioAddress);
            if (encryptKey && encryptKey.encrypt_public_key) {
    ...
    getReceivedFioRequests(options) {
        const { limit, offset, includeEncrypted, encryptKeys, } = options;
    ...
    pushTransaction(options) {
        return __awaiter(this, void 0, void 0, function* () {
            const { account, action, data, authPermission, encryptOptions = {}, signingAccount, } = options;
            data.tpid = this.getTechnologyProviderId(data.tpid);
            if (data.content && !encryptOptions.publicKey) {
                switch (action) {
                    case 'newfundsreq': {
                        const payerKey = yield this.getEncryptKey(data.payer_fio_address);
                        encryptOptions.publicKey = payerKey.encrypt_public_key;
                        encryptOptions.contentType = 'new_funds_content';
                        break;
                    }
                    case 'recordobt': {
                        const payeeKey = yield this.getEncryptKey(data.payee_fio_address);
